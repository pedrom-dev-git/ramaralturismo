...
OUT_DIR = ROOT / "assets" / "images" / "map-patterns"
        ...
        for way in ways:
            if in_bbox(way["coords"], bbox):
                ...
                if d:
                    paths.append(d)
        if paths:
            lines.append(f'<g id="rivers" fill="none" stroke="{river_color}" stroke-width="{river_sw}" stroke-linecap="round" stroke-linejoin="round" opacity="0.7">')
            for d in paths:
        ...
        for way in ways:
            if in_bbox(way["coords"], bbox):
                ...
                if d:
                    paths.append(d)
        if paths:
            lines.append(f'<g id="highways-major" fill="none" stroke="{road_major_color}" stroke-width="{road_major_sw}" stroke-linecap="round">')
            for d in paths:
                lines.append(f'  <path d="{d}"/>')
            lines.append("</g>")
        ...
        for way in ways:
            if in_bbox(way["coords"], bbox):
                ...
                if d:
                    paths.append(d)
        if paths:
            lines.append(f'<g id="highways-primary" fill="none" stroke="{road_primary_color}" stroke-width="{road_primary_sw}" stroke-linecap="round" opacity="0.6">')
            for d in paths:
                lines.append(f'  <path d="{d}"/>')
            lines.append("</g>")
        ...
        for way in ways:
            if in_bbox(way["coords"], bbox):
                ...
                if d:
                    paths.append(d)
        if paths:
            lines.append(f'<g id="highways-secondary" fill="none" stroke="{road_secondary_color}" stroke-width="{road_secondary_sw}" stroke-linecap="round" opacity="0.4">')
            for d in paths:
                lines.append(f'  <path d="{d}"/>')
            lines.append("</g>")
